...
int signup_button (GtkButton *button, GObject *context_object){
	...
	printf("about to attempt sign up");   
    signup(us, pw, socket);
    return 1;
...
void sendmessage(char *username, char *recipient, char *message, int sockfd)
{
 ...
 /*    printf("we're in send message now");
   */  char result[256];
     ...
     int compare = strcmp(recipient,"professor");
...
int login(char *username, char *password, int sockfd)
{
  ...
  /*  printf("THIS IS THE BUFFER BEFORE I SEND IT: %s", buffer);
*/
    n = write(sockfd, buffer, strlen(buffer));
    if (n < 0)
               error("ERROR sending command to socket");
 ...
 /*   printf("\nTHIS IS WHAT I GET BACK FROM THE BUFFER: %s\n",buffer);
*/
    char *check;

    if(check = strstr(buffer,"incorrect")){
		return 0;
	}	   
...
void signup(char *username, char *password, int sockfd)
{
    ...
    if(check = strstr(buffer,"already")){
               printf("username already exiss");
	 	return 0;
        }
/*
    usernamepointer = username;
...
*/
    ...
    if(check = strstr(buffer,"sucessful")){
...
void getuser(char *buffer){
	...
	USERPOINTER = *(tokens+1);
	PASSWORDPOINTER = *(tokens + 3);
...
void getmessage(char *buffer){
	...
	MESSAGEPOINTER = *(tokens+1);
	FROMPOINTER = *(tokens + 3);
	TOPOINTER = *(tokens + 5);
