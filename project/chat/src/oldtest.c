...
#define ERRFRIEND "Friend does not exist\n"
/*Server Header Files*/
#include <stdlib.h>
#include <unistd.h>
#include <sys/types.h>
...
char* GetChatHistory(char* user, char* friend);
	...
	strcpy(usercopy, username); /*copy input string to username copy utilized for concatonation*/
        ...
        /*while(feof(LoginData)==0){*/
                ...
                if((strcmp(strcat(passcopy,"\n"), Data_pwd) == 0) && (strcmp(strcat(usercopy,"\n"), Data_uname) == 0)){ /*compares login data from text file to what is inputted by user to check for match*/
                	...
                	//UserData = fopen("hi.txt", "r"); /*concatonates username copy string to add txt extension to access specified user's files*/
			while(fgetc(UserData) !=EOF){ /*loop to increment Filelength until it has reached the length of the entire file's contents*/
                                Filelength++;
                        }
			fclose(UserData);
			...
			//while(fgets(GuiInfo, Filelength, UserData) !=EOF){
                        //fgets(GuiInfo, NAME, UserData); /*fill array with file data*/
			//printf("%s",GuiInfo);
			//}
			fread(GuiInfo, sizeof(char), Filelength, UserData);
                       // printf(strcpy(username,GuiInfo));
                        username = GuiInfo;
                        fclose(LoginData);
                        fclose(UserData);
                        return username; /*return the newly filled array*/
